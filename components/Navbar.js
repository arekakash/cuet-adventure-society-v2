...
export default function Navbar() {
  ...
  // আপাতত এটি false রাখা হলো, Supabase Auth যুক্ত করার পর এটি ডায়নামিক হবে
  ...
  return (
    <>
            ...
            {!isLoggedIn ? (
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="text-right hidden md:block pr-4 border-r border-white/10">
                  <p className="text-xs font-bold text-gray-200">Explorer</p>
                  <p className="text-[10px] text-campfire uppercase tracking-widest">User</p>
                </div>
                <div className="relative">
                  <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="block w-10 h-10 rounded-full border-2 border-campfire p-0.5 overflow-hidden bg-white/5 focus:outline-none hover:border-white transition-colors shadow-glow">
                    <img src="https://ui-avatars.com/api/?name=User&background=e76f51&color=fff" alt="Profile" className="w-full h-full rounded-full object-cover" />
                  </button>
                  
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-3 w-56 bg-moss border border-white/10 rounded-2xl shadow-2xl py-2 z-50 text-gray-300">
                      <Link href="/dashboard" className="block px-4 py-3 text-sm hover:bg-white/5 hover:text-white transition-colors flex items-center gap-3">
                        <i className="fa-solid fa-user w-5 text-center text-trail"></i> <span>আমার প্রোফাইল</span>
                      </Link>
                      ...
                      <button className="w-full text-left px-4 py-3 text-sm text-red-400 font-bold hover:bg-red-500/10 transition-colors flex items-center gap-3">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-500 ease-in-out z-50 w-72 sm:w-80 bg-moss border-r border-white/5 shadow-2xl flex flex-col h-full overflow-y-auto`}>
        <div className="p-6 flex justify-between items-center border-b border-white/5">
          <span className="font-black text-xl tracking-widest text-white"><span className="text-campfire">C</span>UET <span className="text-campfire">A</span>S</span>
        ...
        <div className="px-6 py-8 flex-grow space-y-2 text-gray-300">
          ...
          <Link href="/events" className="block py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
            <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-campfire group-hover:scale-110 transition-transform"><i className="fa-solid fa-calendar-day"></i></div>
            <span className="font-bold text-sm">আপকামিং ইভেন্ট</span>
          </Link>
          <Link href="/past-events" className="block py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
          ...
          {/* Leaderboard - Uniform Style */}
          <Link href="/leaderboard" className="block py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
          ...
          <Link href="/store" className="block py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
          ...
          <Link href="/beginners-guide" className="block py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
          ...
          <button onClick={() => setIsSettingsOpen(true)} className="w-full text-left py-3 px-4 rounded-xl hover:bg-white/5 hover:text-white transition-all group flex items-center gap-4">
        ...
        {!isLoggedIn && (
          <div className="p-6 border-t border-white/5 glass-dark space-y-3">
            <Link href="/signup" className="w-full flex items-center justify-center gap-2 bg-campfire hover:bg-orange-600 text-white py-3 rounded-xl font-bold transition-all shadow-glow">
              <i className="fa-solid fa-user-astronaut"></i> <span>অ্যাকাউন্ট খুলুন</span>
            </Link>
            <Link href="/login" className="w-full flex items-center justify-center gap-2 border border-white/10 text-gray-300 hover:bg-white/5 py-3 rounded-xl font-bold transition-all">
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-moss border border-white/10 rounded-3xl p-8 max-w-sm w-full mx-4 relative z-10 shadow-2xl">
